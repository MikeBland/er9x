...
#---------------- OPTIONS ---------------------------
# Enable JETI-Telemetry reception on UART0
...
CPPSRC = $(TARGET).cpp stamp.cpp menus.cpp pers.cpp file.cpp lcd.cpp drivers.cpp
...
MSG_COMPILING_JETI = Compiling With JETI DUPLEX enabled
...
VER = ${shell sh -c "grep \"SUB_VERS\" stamp-er9x.h | grep -E \"[[:digit:]]*\""}
VER := $(shell echo ${VER}+1 )

# Default target.
all: begin gccversion sizebefore build sizeafter end

#make with JETI DUPLEX enabled
jeti: ALL_CPPFLAGS += -DJETI
	OBJ += $(OBJDIR)/jeti.o
jeti: jetimsg all
...
build: stamp font.lbm sticks.lbm elf hex eep lss sym
#build: lib
...
stamp:
	...
	@echo "#define TIME_STR \"`date +%H:%I:%S`\"" >> stamp-er9x.h
	@echo "#define SUB_VERS $(VER)-$(USER)" >> stamp-er9x.h
	@echo "#define SVN_VERS \"trunk-$(REV)\"" >> stamp-er9x.h
	@cat stamp-er9x.h
