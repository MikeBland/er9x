...
const prog_char APM Str_telemItems[] = "\004----A1= A2= RSSITSSITim1Tim2Alt GaltGspdT1= T2= RPM FUELMah1Mah2CvltBattAmpsMah CtotFasV" ; 
...
const prog_uint8_t APM TelemValid[] = { 1, 1, 1, 0, 0, 0, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 0, 2, 2, 2, 2 } ;

#else
const prog_char APM Str_telemItems[] = "\004----TIM1TIM2BATT" ;
const prog_int8_t APM TelemIndex[] = { TIMER1, TIMER2, BATTERY } ;
const prog_uint8_t APM TelemValid[] = { 0, 0, 0 } ;
...
void MState2::check(uint8_t event, uint8_t curr, const MenuFuncP *menuTab, uint8_t menuTabSize, const prog_uint8_t *horTab, uint8_t horTabMax, uint8_t maxrow)
{
    ...
    if(scrollLR || scrollUD || p1valdiff) g_LightOffCounter = (g_eeGeneral.lightAutoOff*250) << 1; // on keypress turn the light on 5*100
...
#define RBOX_CENTERY  (SCREEN_HEIGHT-9-BOX_WIDTH/2)
    ...
    DO_SQUARE(LBOX_CENTERX+(cs[0]/((2*RESX)/BOX_LIMIT)), LBOX_CENTERY-(cs[1]/((2*RESX)/BOX_LIMIT)), MARKER_WIDTH);
...
//            lcd_putsnAtt(   18*FW, y, PSTR("---INV")+ld->revert*3,3,attr);
            if(active) {
                CHECK_INCDEC_H_MODELVAR(ld->revert, 0, 1);
            }
            break;
        }
    }
...
uint8_t onoffMenuItem( uint8_t value, uint8_t y, const prog_char *s, uint8_t condition, uint8_t event )
{
    ...
    if(condition) CHECK_INCDEC_H_GENVAR( value, 0, 1);
    return value ;
}
...
uint8_t subN = 1;
	...
	lcd_puts_Pleft(FH, PSTR("UsrProto"));
	{
		...
		{
			attr = INVERS ;
			CHECK_INCDEC_H_MODELVAR(b,0,1); g_model.FrSkyUsrProto = b ;
		}
		...
		{
			attr = INVERS ;
			CHECK_INCDEC_H_MODELVAR(b,0,1); g_model.FrSkyImperial = b ;
		}
...
for (int i=0; i<2; i++) {
    ...
    if (sub==subN && (s_editMode
        ...
        case 1:
            ...
            CHECK_INCDEC_H_MODELVAR( fd->type, 0, 3);
            break;
        }
    }
    subN++; y+=FH;
...
//		uint8_t active = (attr && s_editMode) ;
  	lcd_outdezAtt( 14*FW, 3*FH, value, attr ) ;
		if ( attr == BLINK )
		{
  		g_model.frsky.alarmData[0].frskyAlarmLimit = checkIncDec16( g_model.frsky.alarmData[0].frskyAlarmLimit, 0, 200, EE_MODEL);
		}
...
//		active = (attr && s_editMode) ;
		...
		if ( attr == BLINK )
		{
  		CHECK_INCDEC_H_MODELVAR( g_model.frsky.alarmData[0].frskyAlarmSound, 0, 15 ) ;
		}
  	subN++;
  	...
  	if(sub==subN) { attr = INVERS ; CHECK_INCDEC_H_MODELVAR(g_model.numBlades, 0, 2);}
  	lcd_putcAtt( 13*FW, 4*FH, g_model.numBlades+'2', attr) ;
  	subN++;
	    ...
	    b = g_model.FrSkyAltAlarm ;
	  	...
	  	if(sub==subN) {
				attr = blink ;
  			g_model.FrSkyAltAlarm = CHECK_INCDEC_H_MODELVAR( b, 0, 2);
			}
	...
	else
	{
  	...
  	lcd_puts_Pleft( FH, PSTR("Custom Display") );
		...
		{
			attr = 0 ;
	  	if(sub==subN) { attr = INVERS ; CHECK_INCDEC_H_MODELVAR(  g_model.CustomDisplayIndex[j], 0, NUM_TELEM_ITEMS ) ;}
...
void menuProcTemplates(uint8_t event)  //Issue 73
{
    ...
    if(attr) CHECK_INCDEC_H_GENVAR( g_eeGeneral.templateSetup, 0, 23);
    y+=FH;
 ...
 for(uint8_t i=0; i<7; i++)
 {
  ...
  k=i+t_pgOfs;
    ...
    uint8_t attr = ((sub==k) ? (s_editMode ? BLINK : INVERS) : 0);
 	  ...
 	  if(sub==k)
		{
      CHECK_INCDEC_H_MODELVAR( g_model.numVoice, 0, 16 ) ;
    }
	}
    ...
    putsChn(0,y,k,0);
        ...
        uint8_t attr = ((sub==k && subSub==j) ? (s_editMode ? BLINK : INVERS) : 0);
	      ...
	      if (j == 0)
      	  ...
      	  if(active)
					{
	          CHECK_INCDEC_H_MODELVAR( sd->opt.ss.mode, 0, 2 ) ;
  	      }
				}
      	else if (j == 1)
        {
        ...
        uint8_t attr = ((sub==k && subSub==j) ? (s_editMode ? BLINK : INVERS) : 0);
    		...
    		if (j == 0)
    		  ...
    		  if(active)
					{
    		    CHECK_INCDEC_H_MODELVAR( sd->opt.vs.vswtch, 0, MAX_DRSWITCH-1 ) ;
    		  }
				}
    		else if (j == 1)
    		{
    		  ...
    		  if(active)
					{
    		    CHECK_INCDEC_H_MODELVAR( sd->opt.vs.vmode, 0, 6 ) ;
    		  }
				}
				else
				{
...
//    lcd_putsnAtt( 4*FW , y, PSTR(CSWITCH_STR)+CSW_LEN_FUNC*cs.func,CSW_LEN_FUNC,subSub==0 ? attr : 0);
		lcd_putsAttIdx( 4*FW-3, y, PSTR(CSWITCH_STR),cs.func,subSub==0 ? attr : 0);
...
//#endif
    ...
    if((s_editMode
        ...
        switch (subSub) {
        case 0:
            CHECK_INCDEC_H_MODELVAR( cs.func, 0,CS_MAXF);
...
//#endif
                break;
            case (CS_TIMER):
                CHECK_INCDEC_H_MODELVAR( cs.v1, 0,99);
                break;
...
//#endif
                break;
            case (CS_TIMER):
                CHECK_INCDEC_H_MODELVAR( cs.v2, 0,99);
                break;
            default:
                break;
            }
...
static void insertMix(uint8_t idx)
{
    MixData *md = &g_model.mixData[0] ;
		md += idx ;
...
void menuProcMixOne(uint8_t event)
{
    ...
    for(uint8_t k=0; k<7; k++)
    {
        ...
        case 7:
            ...
            if(attr) { CHECK_INCDEC_H_MODELVAR( b, 0,3); md2->mixWarn = b ; }
            break;
        case 8:
        ...
        case 9:
            ...
            if(attr)  CHECK_INCDEC_H_MODELVAR( md2->delayDown, 0,15); //!! bitfield
            break;
        case 10:
            lcd_puts_Pleft(  y,PSTR("\002Delay Up"));
            lcd_outdezAtt(FW*16,y,md2->delayUp,attr);
            if(attr)  CHECK_INCDEC_H_MODELVAR( md2->delayUp, 0,15); //!! bitfield
            break;
        case 11:
            lcd_puts_Pleft(  y,PSTR("\002Slow  Down"));
            lcd_outdezAtt(FW*16,y,md2->speedDown,attr);
            if(attr)  CHECK_INCDEC_H_MODELVAR( md2->speedDown, 0,15); //!! bitfield
            break;
        case 12:
            lcd_puts_Pleft(  y,PSTR("\002Slow  Up"));
            lcd_outdezAtt(FW*16,y,md2->speedUp,attr);
            if(attr)  CHECK_INCDEC_H_MODELVAR( md2->speedUp, 0,15); //!! bitfield
            break;
        case 13:
...
void genMixTab()
{
    ...
    uint8_t mtIdx   = 0;
    uint8_t sel     = 1;
    memset(Xmem.s_mixTab,0,sizeof(Xmem.s_mixTab));
    ...
    for(uint8_t i=0; i<MAX_MIXERS; i++)
    {
        ...
        if(destCh > maxDst){
            while(destCh > maxDst){ //ch-loop, hole alle channels auf
                maxDst++;
                Xmem.s_mixTab[mtIdx].chId  = maxDst; //mark channel header
                Xmem.s_mixTab[mtIdx].showCh = true;
                Xmem.s_mixTab[mtIdx].selCh = sel++; //vorab vergeben, falls keine dat
                Xmem.s_mixTab[mtIdx].insIdx= i;     //
                mtIdx++;
            }
        ...
        Xmem.s_mixTab[mtIdx].chId    = destCh; //mark channel header
        Xmem.s_mixTab[mtIdx].editIdx = i;
        Xmem.s_mixTab[mtIdx].hasDat  = true;
        Xmem.s_mixTab[mtIdx].selDat  = sel++;
        if(md[i].destCh == md[i+1].destCh){
            Xmem.s_mixTab[mtIdx].selCh  = 0; //ueberschreibt letzte Zeile von ch-loop
            Xmem.s_mixTab[mtIdx].insIdx = 0; //
        }
        else{
            Xmem.s_mixTab[mtIdx].selCh  = sel++;
            Xmem.s_mixTab[mtIdx].insIdx = i+1; //
        }
        s_mixMaxSel =sel;
        mtIdx++;
    }
...
static void moveMix(uint8_t idx,uint8_t mcopy, uint8_t dir) //true=inc=down false=dec=up - Issue 49
{
...
void menuMixersLimit(uint8_t event)
{
    ...
    lcd_outdez(20*FW, 2*FH, getMixerCount() );
    ...
    for(i=0; i<7; i++){
        ...
        if(mixtabptr->hasDat){ //show data
            ...
            uint8_t attr = sub==mixtabptr->selDat ? INVERS : 0;
						uint8_t tattr = s_moveMode ? attr : 0 ;
            ...
            if(attr == INVERS){ //handle dat is selected
                CHECK_INCDEC_H_MODELVAR( md2->weight, -125,125);
...
if(t_pgOfs<subN) {
...
#endif
					 event==EVT_KEY_FIRST(KEY_DOWN) || event==EVT_KEY_FIRST(KEY_UP) || event==EVT_KEY_REPT(KEY_DOWN) || event==EVT_KEY_REPT(KEY_UP))
            CHECK_INCDEC_H_MODELVAR( v ,0,NUMCHARS-1);
        v = idx2char(v);
    ...
    if(sub==subN)
		{
			attr = INVERS ;
      CHECK_INCDEC_H_MODELVAR( g_model.modelVoice ,0,49 ) ;
		}
...
if(t_pgOfs<subN) {
    ...
    if(sub==subN && (s_editMode
      ...
      int8_t min=qr.quot ;
        ...
        case 0:
        {
            CHECK_INCDEC_H_MODELVAR( min ,0,59);
...
if(t_pgOfs<subN) {
    lcd_puts_Pleft(    y, Str_Timer);
    uint8_t attr = 0 ;
    if(sub==subN) { attr = INVERS ; CHECK_INCDEC_H_MODELVAR(g_model.tmrDir,0,1); }
...
if(t_pgOfs<subN) {
    ...
    if(sub==subN) { CHECK_INCDEC_H_MODELVAR(b,0,1); g_model.thrTrim = b ; }
    if((y+=FH)>7*FH) return;
}subN++;
...
if(t_pgOfs<subN) {
    ...
    menu_lcd_onoff( 10*FW, y, b, sub==subN ) ;
    if(sub==subN) { CHECK_INCDEC_H_MODELVAR(b,0,1); g_model.thrExpo = b ; }
    if((y+=FH)>7*FH) return;
}subN++;
...
if(t_pgOfs<subN) {
    ...
    uint8_t attr = 0 ;
    if(sub==subN) { attr = INVERS ; CHECK_INCDEC_H_MODELVAR(g_model.trimInc,0,4);}
...
if(t_pgOfs<subN) {
										...
										))
    {
        ...
        switch (subSub){
        case 0:
            CHECK_INCDEC_H_MODELVAR(g_model.protocol,0, prot_max ) ;
            break;
        case 1:
            if ((g_model.protocol == PROTO_PPM) || (g_model.protocol == PROTO_PPM16)|| (g_model.protocol == PROTO_PPMSIM) )
...
if(t_pgOfs<subN) {
    ...
    else
    {
        ...
        if(sub==subN) CHECK_INCDEC_H_MODELVAR(g_model.ppmNCH,0,2);
    }
    if((y+=FH)>7*FH) return;
}subN++;
...
if(t_pgOfs<subN) {
    ...
    if(sub==subN) { attr = INVERS ; CHECK_INCDEC_H_MODELVAR(g_model.pulsePol,0,1);}
...
if(t_pgOfs<subN) {
    ...
    menu_lcd_onoff( 10*FW, y, b, sub==subN ) ;
    if(sub==subN) { CHECK_INCDEC_H_MODELVAR(b,0,1); g_model.extendedLimits = b ; }
    if((y+=FH)>7*FH) return;
}subN++;
...
if(t_pgOfs<subN) {
    ...
    menu_lcd_onoff( 10*FW, y, b, sub==subN ) ;
    if(sub==subN) { CHECK_INCDEC_H_MODELVAR(b,0,1); g_model.traineron = b ; }
    if((y+=FH)>7*FH) return;
}subN++;
...
if(t_pgOfs<subN) {
    ...
    menu_lcd_onoff( 10*FW, y, b, sub==subN ) ;
    if(sub==subN) { g_model.t2throttle =CHECK_INCDEC_H_MODELVAR(b,0,1); }
    if((y+=FH)>7*FH) return;
}subN++;
...
uint8_t subN = 1;
    ...
    lcd_puts_Pleft(    y, PSTR("Swash Type"));
		attr = 0 ;
    if(sub==subN) {attr = INVERS ; CHECK_INCDEC_H_MODELVAR(b,0,SWASH_TYPE_NUM); g_model.swashType = b ; }
...
subN++;
    ...
    lcd_puts_Pleft(    y, PSTR("Collective"));
		attr = 0 ;
    if(sub==subN) {attr = INVERS ; CHECK_INCDEC_H_MODELVAR( g_model.swashCollectiveSource, 0, NUM_XCHNRAW);}
...
subN++;
    ...
    lcd_puts_Pleft(    y, PSTR("Swash Ring"));
		attr = LEFT ;
    if(sub==subN) {attr = INVERS|LEFT ; CHECK_INCDEC_H_MODELVAR( g_model.swashRingValue, 0, 100);}
...
subN++;
    ...
    if(sub==subN) { CHECK_INCDEC_H_MODELVAR( b, 0, 1); g_model.swashInvertELE = b ; }
    if((y+=FH)>7*FH) return;
subN++;
    ...
    menu_lcd_HYPHINV( 14*FW, y, b, sub==subN ) ;
    if(sub==subN) { CHECK_INCDEC_H_MODELVAR( b, 0, 1); g_model.swashInvertAIL = b ; }
    if((y+=FH)>7*FH) return;
subN++;
    ...
    menu_lcd_HYPHINV( 14*FW, y, b, sub==subN ) ;
    if(sub==subN) { CHECK_INCDEC_H_MODELVAR(b, 0, 1); g_model.swashInvertCOL = b ; }
//    if((y+=FH)>7*FH) return;
...
//    edit = (sub==i && subSub==0);
			switch(j)
			{
    			...
    			if (attr&BLINK)
        	{ CHECK_INCDEC_H_GENVAR( x, 0, 2); td->mode = x ; } //!! bitfield
				break ;
    			...
    			if (attr&BLINK)
		        { CHECK_INCDEC_H_GENVAR( x, 0, 3); td->srcChn = x ; } //!! bitfield
				break ;
...
//    if(s_pgOfs==vCountItems-7) s_pgOfs= sub<(vCountItems-4) ? vCountItems-8 : vCountItems-6;
...
#endif
							 event==EVT_KEY_FIRST(KEY_DOWN) || event==EVT_KEY_FIRST(KEY_UP) || event==EVT_KEY_REPT(KEY_DOWN) || event==EVT_KEY_REPT(KEY_UP))
                CHECK_INCDEC_H_GENVAR( v ,0,NUMCHARS-1);
            v = idx2char(v);
    ...
    if(t_pgOfs<subN) {
        ...
        if(sub==subN) { attr = INVERS ; CHECK_INCDEC_H_GENVAR( b, 0, 6); g_eeGeneral.beeperVal = b ; }
    ...
    if(t_pgOfs<subN) {
        ...
        if(sub==subN) { attr = INVERS ; CHECK_INCDEC_H_GENVAR( b, 0, 3); g_eeGeneral.speakerMode = b ; }
...
if((g_eeGeneral.speakerMode & 1) == 1 /*|| g_eeGeneral.speakerMode == 2 */){
    ...
    if(t_pgOfs<subN) {
        ...
        if(sub==subN) { attr = INVERS | LEFT ;CHECK_INCDEC_H_GENVAR( g_eeGeneral.inactivityTimer, -10, 110); } //0..120minutes
        lcd_outdezAtt(PARAM_OFS, y, g_eeGeneral.inactivityTimer+10, attr);
    ...
    if(t_pgOfs<subN) {
        g_eeGeneral.blightinv = onoffMenuItem( g_eeGeneral.blightinv, y, PSTR("Backlight invert"), sub==subN, event ) ;
        if((y+=FH)>7*FH) return;
    }subN++;
    ...
    if(t_pgOfs<subN) {
        ...
        lcd_puts_Pleft( y,( i == 0) ? PSTR("Light off after") : PSTR("Light on Stk Mv") );
  			...
  			uint8_t attr = 0 ;
        if(sub==subN) { attr = INVERS ; CHECK_INCDEC_H_GENVAR( b, 0, 600/5);}
        if(b) {
...
// Try this instead of the switch, might sae 26 bytes flash
...
//frsky alert mappings
    ...
    if(t_pgOfs<subN) {
        ...
        if((y+=FH)>7*FH) return;
  			...
  			uint8_t attr = 0 ;
        if(sub==subN) { attr = INVERS ; CHECK_INCDEC_H_GENVAR(g_eeGeneral.stickMode,0,3);}
	      					...
	      					for (uint8_t k=0; k<FrskyBattCells; k++)
									{
  									...
  									lcd_outdezNAtt( x, y, (uint8_t)(FrskyVolts[k] * 2) , blink, 4 ) ;
										x += 7*FW ;
...
#define REZ_SWASH_Y(x)  ((x))   //  1024 => 1024
        ...
        {
            int16_t vp = Output.anas[ele_stick]+trimA[ele_stick];
            int16_t vr = Output.anas[ail_stick]+trimA[ail_stick];
...
#define DEL_MULT 256
            ...
            //swTog = false;
            //}
            int16_t diff = v-Output.act[i]/DEL_MULT;
            ...
            if(Output.sDelay[i]){ // perform delay
                if(tick10ms)
                {
                  Output.sDelay[i]-- ;
                }
                if (Output.sDelay[i] != 0)
                { // At end of delay, use new V and diff
                  v = Output.act[i]/DEL_MULT;   // Stay in old position until delay over
                  diff = 0;
                }
            }
...
//										act[i] = (speed) ? act[i]+(rate)/((int16_t)100*speed) : (int32_t)v*DEL_MULT ;
										...
										Output.act[i] = (diff>0) ? ((md->speedUp>0)   ? Output.act[i]+(rate)/((int16_t)100*md->speedUp)   :  (int32_t)v*DEL_MULT) :
                                        ((md->speedDown>0) ? Output.act[i]-(rate)/((int16_t)100*md->speedDown) :  (int32_t)v*DEL_MULT) ;
                }
								{
									int32_t tmp = Output.act[i]/DEL_MULT ;
                	if(((diff>0) && (v<tmp)) || ((diff<0) && (v>tmp))) Output.act[i]=(int32_t)v*DEL_MULT; //deal with overflow
								}
                v = Output.act[i]/DEL_MULT;
            }
            else if (diff)
            {
              Output.act[i]=(int32_t)v*DEL_MULT;
            }
        }
