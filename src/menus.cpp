...
const prog_char APM Str_telemItems[] = "\004----A1= A2= RSSITSSITim1Tim2Alt GaltGspdT1= T2= RPM FUELMah1Mah2CvltBattAmpsMah CtotFasVAccXAccYAccZVspdGvr1Gvr2Gvr3Gvr4Gvr5Gvr6Gvr7Fwat" ; 
...
#else
const prog_char APM Str_telemItems[] = "\004----TIM1TIM2BATTGvr1Gvr2Gvr3Gvr4Gvr5Gvr6Gvr7" ;
...
//        if(s_editMode && subSub>0) g_posHorz--;
//        break;
//    case EVT_KEY_REPT(KEY_RIGHT):
//    case EVT_KEY_FIRST(KEY_RIGHT):
//        if(s_editMode && subSub<(cv9 ? 9 : 5)) g_posHorz++;
//        break;
//}

//s_editMode = g_posHorz;
...
//lcd_putsAtt( 2*FW, 1*FH,PSTR("EDIT"),((sub == -1)) ? INVERS : 0);
...
//		active = (attr && s_editMode) ;
	...
	{
		uint8_t subN = 14 ;
  	lcd_puts_Pleft( FH, PSTR("FAS Offset") );
    uint8_t attr = PREC1 ;
		if ( (sub == subN) )
		{
			attr = INVERS | PREC1 ;
   		...
   		if (j == 0)
			{
				...
				lcd_putsAttIdx( 15*FW, y, PSTR("\004----vspdA2  "), g_model.varioData.varioSource, attr ) ;
   		  if(attr)
				{
...
//          lcd_outdezAtt( 20*FW, y, convertTelemValue( cs.v1-CHOUT_BASE-NUM_CHNOUT-1, cs.v2 ) ,subSub==2 ? attr : 0);
					putsTelemetryChannel( 18*FW-8, y, cs.v1-CHOUT_BASE-NUM_CHNOUT-1, value, subSub==2 ? attr : 0, TELEM_UNIT ) ;
				}
        else
//#endif
...
int8_t edit_dr_switch( uint8_t x, uint8_t y, int8_t drswitch, uint8_t attr, uint8_t edit )
{
	...
	if(edit) CHECK_INCDEC_H_MODELVAR( drswitch, -MAX_DRSWITCH, MAX_DRSWITCH) ;
	return drswitch ;
}
...
void menuProcMixOne(uint8_t event)
{
    ...
    uint8_t x = TITLEP( PSTR("EDIT MIX "));
//    uint8_t x = TITLEP(s_currMixInsMode ? PSTR("INSERT MIX ") : PSTR("EDIT MIX "));
    ...
    for(uint8_t k=0; k<7; k++)
    {
        ...
        case 2:
            ...
            break;
        case 3:
						md2->lateOffset = onoffMenuItem_m( md2->lateOffset, y, PSTR("\002Fix Offset"), attr ) ;
            break;
        case 4:
...
const prog_char APM MixPopList[] = "EDIT\0INSERT\0COPY\0MOVE\0DELETE" ;
...
void menuProcExpoAll(uint8_t event)
{
    ...
    uint8_t attr = 0 ;
		if ( sub == 1 )
		{
			s_expoChan = checkIncDec( s_expoChan, 0, 3, 0 ) ;
			attr = INVERS ;
		}		 
    ...
    break;
    default: // DR_HIGH:
      lcd_puts_Pleft(FH,PSTR("\004DR Hi"));
    break;
	}
...
void menuPhaseOne(uint8_t event)
{
  ...
  for (uint8_t i = 0 ; i < 2 ; i += 1 )
    ...
    uint8_t y = (i+1) * FH;
      ...
      case 1 : // trims
				lcd_puts_Pleft( y, PSTR("Trims") ) ;
        for ( uint8_t t = 0 ; t<NUM_STICKS ; t += 1 )
				{
          putsTrimMode( (8+t)*FW, y, s_currIdx+1, t, (g_posHorz==t) ? attr : 0 ) ;
#ifndef NOPOTSCROLL
...
uint8_t ModelPopup ;
const prog_char APM ModelPopList[] = "SELECT\0COPY\0MOVE\0DELETE" ;
...
//    edit = (sub==i && subSub==0);
        	...
        	{ CHECK_INCDEC_H_GENVAR_0( x, 2); td->mode = x ; } //!! bitfield
    			...
    			if (attr&BLINK)
        		{ CHECK_INCDEC_H_GENVAR( x, -15, 15); td->swtch = x ; }
				break ;
			}
		}
   	y += FH;
...
//        break;
...
#ifndef NOPOTSCROLL
        if(!s_editMode && scrollLR<0) { s_editMode = true; scrollLR = 0; g_posHorz = 0; }
        if(s_editMode && scrollLR>subSub) { s_editMode = false; scrollLR = 0; g_posHorz = 0; }
#endif
        if(sub==subN && s_editMode){
#ifndef NOPOTSCROLL
            g_posHorz -= scrollLR;
#endif            
						if((int8_t(g_posHorz))<0) g_posHorz = 0;
            if((int8_t(g_posHorz))>(GENERAL_OWNER_NAME_LEN-1)) g_posHorz = GENERAL_OWNER_NAME_LEN-1;
#ifndef NOPOTSCROLL
            scrollLR = 0;
#endif
    ...
    if(t_pgOfs<subN) {
        ...
        lcd_putsAttIdx( 11*FW, y, PSTR("\012Beeper    ""PiSpkr    ""BeeprVoice""PiSpkVoice""MegaSound "),b,attr);
...
#ifndef NOPOTSCROLL
            if(event==EVT_KEY_FIRST(KEY_MENU) || event==EVT_KEY_FIRST(BTN_RE) || P1values.p1valdiff )
#else
    ...
    if(t_pgOfs<subN)
    {
		...
		if(t_pgOfs<subN)
    {
        g_eeGeneral.stickScroll = onoffMenuItem_g( g_eeGeneral.stickScroll, y, PSTR("StickScroll"), sub==subN ) ;
        if((y+=FH)>7*FH) return;
    }subN++;
...
void switchDisplay( uint8_t j, uint8_t a )
{
	uint8_t b = a + 3 ;
	uint8_t y = 4*FH ;
	for(uint8_t i=a; i<b; y += FH, i += 1 ) lcd_putsnAtt((2+j*15)*FW-2, y, Str_Switches+3*i,3,getSwitch(i+1, 0) ? INVERS : 0);
}
...
//                            blink = (alarmRaised[i] ? INVERS : 0);
            ...
            else if ( tview == 0x20 )
            {
                if (frskyUsrStreaming)
                {
                  ...
                  lcd_puts_Pleft( 2*FH, PSTR("\012RPM"));
...
//                    blink = (alarmRaised[1] ? INVERS+BLINK : 0);
            ...
            else if ( tview == 0x30 )
            {
              ...
              if (frskyUsrStreaming)
                ...
                lcd_outdezNAtt(8*FW, 2*FH, getTelemetryValue(FR_GPS_LAT), LEADING0 | blink, -5);
                lcd_putc(8*FW, 2*FH, '.') ;
                lcd_outdezNAtt(12*FW, 2*FH, getTelemetryValue(FR_GPS_LATd), LEADING0 | blink, -4);
                lcd_puts_Pleft( 3*FH, PSTR("Lon=")) ;
                lcd_outdezNAtt(8*FW, 3*FH, getTelemetryValue(FR_GPS_LONG), LEADING0 | blink, -5);
                lcd_putc(8*FW, 3*FH, '.') ;
